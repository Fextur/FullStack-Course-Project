...
export const posts: Post[] = [
  ...Array.from({ length: 70 }, (_, i) => ({
    ...
    ][Math.floor(Math.random() * 4)], // Random image from existing ones
    content: i.toString(), // Placeholder content
    likes: Math.floor(Math.random() * 20), // Random likes count
    user: [idoUser, chrisUser, aviciiUser, testUser][
      Math.floor(Math.random() * 4)
    ], // Random user
    commentsCount: Math.floor(Math.random() * 10), // Random comments count below 10
    isUserLiked: Math.random() < 0.5, // Random true or false
  })),
];
